...
# /home/neel/mnt_data/greenstand/old_deeplab/segmentation_model_build/download_thread.py
dotenv_path = Path('/home/neel/mnt_data/greenstand/old_deeplab/.env')
...
def download_and_process_files(bucket_name, s3_directory, local_directory):
    ...
    def download_file(content):
        try:
            ...
            # Skip files not in 'samples/' or 'binary_masks/'
            if not (
                s3_object_key.startswith(f"{args.s3_directory}{args.samples_folder_name}/") or
                s3_object_key.startswith(f"{args.s3_directory}{args.masks_folder_name}/")
            ):
                return None  # skip unrelated folders
...
if __name__ == "__main__":
    ...
    parser.add_argument("--s3_directory", default='india_sam_dino_annotations_large/', type=str, help="Directory path in the S3 bucket")
    parser.add_argument("--local_directory", default='/home/neel/mnt_data/greenstand/old_deeplab/segmentation_model_build/Deeplab/india_sam_dino_annotations_large', type=str, help="Local directory path to save the downloaded files")
    parser.add_argument("--masks_folder_name", default='masks', type=str, help="Name of the masks folder in the s3 bucket")
