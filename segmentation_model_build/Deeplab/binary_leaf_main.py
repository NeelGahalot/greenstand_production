...
class LeafDataset_old(data.Dataset):
...
#             self.images = [os.path.join(image_dir, fname + '.jpg') for fname in file_names if fname[:16] == 'tomato_unhealthy' or fname[:14] == 'tomato_healthy'
# ]
#             self.masks = [os.path.join(mask_dir, fname + '_mask_.jpg') for fname in file_names if fname[:16] == 'tomato_unhealthy' or fname[:14] == 'tomato_healthy'
#]
            # for fname in file_names if fname[:16]!= 'potato_unhealthy' and fname[:16]!= 'pepper_unhealthy'
            # if fname[:16] != 'potato_unhealthy' and fname[:16] != 'pepper_unhealthy' and fname[:14] != 'pepper_healthy'  and fname[:14] != 'potato_healthy'
            #if fname[:16] != 'tomato_unhealthy' and fname[:14] != 'tomato_healthy'
...
def get_argparser():
    ...
    parser.add_argument("--loss_type", type=str, default='binary_dice',
...
def get_dataset(opts):
    ...
    if opts.dataset == 'custom':
        ...
        train_dst = LeafDataset_old(root=opts.custom_data_path, image_set='train', img_transform=train_img_transform, mask_transform=train_mask_transform)
        val_dst = LeafDataset_old(root=opts.custom_data_path, image_set='val', img_transform=val_img_transform, mask_transform=val_mask_transform)
