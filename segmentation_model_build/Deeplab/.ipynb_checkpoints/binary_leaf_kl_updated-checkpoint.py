...
class CombinedLoss(nn.Module):
    def __init__(self, cosine_weight=1.0, dice_weight=1.0):
        super(CombinedLoss, self).__init__()
        self.cosine_weight = cosine_weight
        self.dice_weight = dice_weight
        self.dice_loss_fn = DiceBCELoss()

    def cosine_similarity_loss(self, student_probs, teacher_probs):
        """
        Compute the Cosine Similarity loss between student and teacher probabilities.
        """
        # Flatten
        student_flat = student_probs.view(student_probs.size(0), -1)
        teacher_flat = teacher_probs.view(teacher_probs.size(0), -1)
        ...
        cos_sim = F.cosine_similarity(student_flat, teacher_flat, dim=1)  # shape: [batch]
        
        # Convert similarity to loss
        return 1 - cos_sim.mean()

    def forward(self, student_logits, teacher_probs, inputs, targets):
        # Get sigmoid probabilities from student logits
        student_probs = torch.sigmoid(student_logits)

        # Cosine loss between student and teacher outputs
        cosine_loss = self.cosine_similarity_loss(student_probs, teacher_probs)

        # Dice+BCE loss using ground truth labels
        dice_loss = self.dice_loss_fn(inputs, targets)

        # Combined loss
        total_loss = self.cosine_weight * cosine_loss + self.dice_weight * dice_loss
...
def get_argparser():
    ...
    parser.add_argument("--loss_type", type=str, default='kl_dice_combined',
    ...
    parser.add_argument("--custom_data_path", type=str, default='crf_sam_annotations_large',
...
def main():
    ...
    if opts.test_only:
        ...
        val_score, ret_samples, _ = validate(
            opts=opts, model=model, loader=val_loader, device=device, metrics=metrics, criterion=DiceBCELoss(), ret_samples_ids=vis_sample_id)
    ...
    while True:  # cur_itrs < opts.total_itrs:
        ...
        for (images, labels,probs) in train_loader:
            ...
            # Label smoothing
            labels = smooth_labels(labels, smoothing=0.3)
            ...
            if (cur_itrs) % opts.val_interval == 0:
                ...
                print(interval_vals)
                save_ckpt('saved_models/kl_check_temp_2_weighted.pth')
                print('saved here')

                
                if val_score['IoU Foreground'] > best_score:  # save best model
                    best_score = val_score['IoU Foreground']
                    #save_ckpt('saved_models/crf_sam_large_freetown_eastafrica_haiti_sam_threshold_95_with_prob_%s.pth' % ((val_score['IoU Foreground'])))

                    #save_ckpt('saved_models/best_%s_%s_os%d_%s.pth' %
                              #(opts.model, opts.dataset, opts.output_stride, (val_score['IoU Foreground'])))
